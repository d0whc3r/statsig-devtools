    ...
    <div className="border-b border-gray-200 py-2">
      <div className="flex items-center justify-between text-xs text-gray-600">
        <div className="flex items-center gap-4">
          <span>Gates: {statistics.configurationsByType.feature_gate || 0}</span>
          <span>Configs: {statistics.configurationsByType.dynamic_config || 0}</span>
          <span>Experiments: {statistics.configurationsByType.experiment || 0}</span>
          {activeOverrides.length > 0 && (
            <span className="font-medium text-blue-600">Overrides: {activeOverrides.length}</span>
          )}
        </div>
