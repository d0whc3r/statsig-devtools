...
export function ExperimentCard({ experiment, onOverride }: ExperimentCardProps) {
  ...
  const isOverrideVisible = showOverride === experiment.id

  const handleToggleExpanded = async () => {
    const newExpandedId = isExpanded ? null : experiment.id
    setExpandedExperiment(newExpandedId)

    // Fetch experiment details when expanding
    if (newExpandedId && !experimentDetails) {
  ...
  const handleToggleOverride = () => {
    setShowOverride(isOverrideVisible ? null : experiment.id)
  }
